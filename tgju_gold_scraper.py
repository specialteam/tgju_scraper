"""
اسکریپر قیمت طلای سایت tgju.org
...
class TGJUGoldScraper:
    """کلاس برای اسکراپ کردن قیمت طلا از سایت tgju.org"""
    
    def __init__(self):
        self.base_url = "https://www.tgju.org/gold-chart"
    ...
    def parse_gold(self, html_content: str) -> List[Dict]:
        """پارس کردن HTML و استخراج قیمت طلا"""
        ...
        for table in tables:
            ...
            for row in rows:
                ...
                try:
                    ...
                    header_keywords = ['قیمت طلا', 'مظنه', 'مثقال طلا', 'قیمت آبشده', 'قیمت نقره', 
                                     'طلا در بورس', 'قیمت زنده', 'تغییر', 'کمترین', 'بیشترین', 'زمان']
                    
                    # بررسی اینکه آیا این یک ردیف header جدول است (عنوان جدول + "قیمت زنده" در سلول دوم)
                    ...
                    if is_header_row:
                        ...
                        if 'قیمت طلا' in name and 'آبشده' not in name and 'نقره' not in name:
                            current_category = 'قیمت طلا'
                        elif 'مظنه' in name or ('مثقال' in name and 'آبشده' not in name):
                            current_category = 'مظنه / مثقال طلا'
                        elif 'آبشده' in name and 'قیمت' in name:
                            current_category = 'قیمت آبشده'
                        elif 'نقره' in name:
                            current_category = 'قیمت نقره'
                        elif 'بورس' in name:
                            current_category = 'طلا در بورس'
                        
                        continue
                    
                    # فیلتر کردن ردیف‌های header
                    if (name in ['قیمت طلا', 'قیمت زنده', 'تغییر', 'کمترین', 'بیشترین', 'زمان'] or
                        all(keyword in name for keyword in ['قیمت', 'زنده']) and 'طلا' not in name):
                        continue
                    ...
                    if change_text:
                        ...
                        # ممکن است شامل نماد هم باشد (مثلاً 8,400 یا 0)
                    ...
                    invalid_names = ['قیمت طلا', 'قیمت زنده', 'تغییر', 'کمترین', 'بیشترین', 'زمان',
                                   'مظنه / مثقال طلا', 'قیمت آبشده', 'قیمت نقره', 'طلا در بورس', 'طلا',
                                   'جدول نرخ های امروز']
                    ...
                    if current_category:
                        gold_item['category'] = current_category
    ...
    def scrape(self) -> List[Dict]:
        """متد اصلی برای اسکراپ کردن قیمت طلا"""
        print("در حال دریافت صفحه...")
        response = self.fetch_page()
        
        print("در حال پارس کردن داده‌ها...")
        gold_items = self.parse_gold(response.text)
        
        return gold_items
    
    def save_to_json(self, data: List[Dict], filename: str = 'gold.json'):
        """ذخیره داده‌ها در فایل JSON"""
        with open(filename, 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
        print(f"داده‌ها در فایل {filename} ذخیره شدند.")
    
    def print_gold(self, gold_items: List[Dict]):
        """چاپ قیمت طلا در کنسول"""
        if not gold_items:
...
def main():
    """تابع اصلی"""
    scraper = TGJUGoldScraper()
    
    try:
        gold_items = scraper.scrape()
        
        if gold_items:
            scraper.print_gold(gold_items)
            scraper.save_to_json(gold_items)
            print(f"\n✓ تعداد {len(gold_items)} آیتم طلا پیدا شد.")
        else:
            ...
            with open('gold_page_source.html', 'w', encoding='utf-8') as f:
                f.write(response.text)
            print("HTML صفحه در فایل gold_page_source.html ذخیره شد.")
